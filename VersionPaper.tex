...
\csvautotabular{tables/project_info_analysis_results.csv}
\end{adjustbox}
\caption{Summary of the projects in our corpus. \TODO{put std. dev. as the last column. Application size [MB]}.}
...
%by referring to them as having a depth equal to the number of version compounds a version number within that tree would contain. For example, the metatree with a root node of x.x is called a metatree of depth 2 and a metatree with a root node of x.x.x.x is referred to as a metatree of depth 4.
...
%In the previous example for empty jumps, the project had an empty jump (1) from 1.0.25 to 1.1 and later has an empty jump (2) from 1.3 to 1.3.1.
%Before the first empty jump, the \choices were 1.0.26, 1.1.0, and 2.0.0.
%After the empty jump the list of \choices also includes 1.2 and 2.0, adding a total of two additional \choices.
%Before the second empty jump the developers still have 5 \choices: 1.0.26, 1.1.0, 2.0.0, 1.4, and 2.0. 
%After the second empty jump the developers have 1.0.26, 1.1.0, 1.3.2, 1.4.0, 2.0.0, 1.4, and 2.0.
%As we explained in Section~\ref{asdfsadF}, the choices 1.3.2 and 1.4.0 emerge due to megalomania.
%If we only consider the effect of empty jumps, the number of choices remains the same (5) and will remain the same with each subsequent empty jump between two and three version compounds. 
%If constructed out of list of version numbers that contains empty jump, metatree represents available \choices introduced by empty jumps as metatree nodes. 
%For example, as shown in Fig... , metatree for list of version numbers ... contains 7 nodes but only 5 nodes are intro. 
%, the software project has the version number list 1.0, 1.1, 1.2. 
%If we assign version numbers 1.0.0, 1.0.1, 1.0.2 to the next three subsequent releases, then the \numberchoices is now 1.3, 2.0, 1.0.3, 1.1.0, and 2.0.0. 
...
After the second empty jump the developers can choose from these \choices 1.0.6, 1.1.0, 2.0.0, 1.2, and 2.0, for the same number of total choices, five.
...
%\TODO{rewrite example with 1.0 -> 1.0.1 -> 1.1: bamboo}

%As shown above, the \numberchoices introduced by empty jumps vary depending on the number of version compounds in an empty jump and 
%
%
%
...
%\TODO{Show concrete example Figure}
It is convenient to use a metatree to represent the mechanics of how empty jumps introduce new \choices.

The representation of an empty jump as a union of two metatrees allows us to account for all possible scenarios for empty jumps between version numbers that consist of an arbitrary number of version compounds.% while only introducing the additional number of choices directly related to empty jumps. 
%To generate the union we take the root node for each metatree involved an empty jump and convert them into children nodes 
We construct two or more metatrees as a result of version number sets that contain empty jumps. 
We refer to the new structure that consists of those two metatrees as a \emph{union metatree}.% (See Figure~\ref{fig:emptyJumpMeta}).
...
%of the union metatree root node, represented by the number of version compounds of both submetatrees and additional version compound in parenthesis. 
The root node of the union metatree is x.x(.x) because it represents variable number of version compounds.
%for both the version numbers with two version compounds and the version numbers with three version compounds and the union metatree as a result of an empty jump. 
...
%\TODO{Megalomania and metatrees}
...
For example, instance of megalomania 1.2.1 -> 2.0.0 has severity 2 because it introduces the \choices 2.0.1 and 2.1.0. 
...
The version set 2.0.x represents the \choice 2.0.1 and the version set 2.x.x represents the \choice 2.1.0. 
...
%\TODO{aggregated empty jumps}

\MyParagraph{Aggregated empty jumps}
...
%\TODO{Meta tree diagrams for the example  }
...
\MyParagraph{\textbf{Aggregated severity of megalomania}}.  
...
%, because the degree of severity represent the number of nodes added to the metatree. 
...
%the sum represents the number of metatree nodes introduced by megalomania.
...
\MyParagraph{Aggregate inconsistency score}
...
\TODO{1. check Aggregate Metrics, 2. Analysis, 3.Implications, 4. Related Work.  5. check Aggregated Inconsistancy Metric}
...
\MyParagraph{Implications for managers}
...
\\
\MyParagraph{Implications for build managers}
It is more difficult to automate releases of proprietary software then open-source projects.
\\
\MyParagraph{Implications for developers}
More changes are likely to be introduced by change of version in proprietary software then open-source software. Also there are more hidden versions - \TODO{so what?}
\\
\MyParagraph{Implications for testers}
Testers need to test more with change in proprietary software version number then with change in open-source 
\\
\MyParagraph{Implications for researchers}
\\

\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/oss_info_analysis_results.csv}
\end{adjustbox}
...
\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/proprietary_info_analysis_results.csv}
\end{adjustbox}
...
\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/oss_analysis_results.csv}
\end{adjustbox}
\end{table*}
...
\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/age_analysis_results.csv}
\end{adjustbox}
\end{table*}
...
\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/team_size_analysis_results.csv}
\end{adjustbox}
\end{table*}
...
%\begin{table*}[htb]
%\begin{adjustbox}{width=\textwidth}
%\csvautotabular{tables/devtools_analysis_results.csv}
%\end{adjustbox}
%\end{table*}
...
\begin{table*}[htb]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/sdk_analysis_results.csv}
\end{adjustbox}
\end{table*}
...
%\begin{table*}[h]
%\begin{adjustbox}{width=\textwidth}
%\csvautotabular{tables/vcs_analysis_results.csv}
%\end{adjustbox}
%\end{table*}

\begin{table*}[h]
\begin{adjustbox}{width=\textwidth}
\csvautotabular{tables/application_size_analysis_results.csv}
\end{adjustbox}
\end{table*}
...
%    \csvreader{tables/oss_analysis_results.csv}{}% use head of csv as column names
...
\bibliography{bibliography,PropVersionRef,ZoteroRefs,OSSRefs}  % sigproc.bib is the name of the Bibliography in this case
