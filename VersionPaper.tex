...
\MyParagraph{\textbf{Length of jumps}}.  For each version compound of a project, this metric counts the sum of all sizes of jumps within that compound. 
...
For example, one project where version 1.0.25 is followed immediately by version 1.1 represents an empty jump, later in this project another empty jump happens from version 1.3 to 1.3.1. We do not distinguish? between adding or removing version compounds because either expanding or shrinking the version number demonstrates a lack of consistency. \TODO{address textual stuff?} \\

%\begin{itemize}

\MyParagraph{\textbf{Number of empty jumps}}. For each version compound of a project, this metric counts the sum of all occurrences of empty jumps throughout all of the projects.
For example, 1.0.25 -> 1.1 and 1.3->1.3.1 is \feedback{(an empty jump in the second and third version compounds, respectively | two empty jumps in the third compound)}. \\
...
\MyParagraph{\textbf{Number of version placeholders}}. We define a version place holder as a special character that represents set of version numbers instead of just one version number. For example, version number 1.x represents set of version numbers [1.0, 1.1, 1.2, $\dots$, 1.N]. 
In our data set, we found only the character 'x' used as a version placeholder, and therefore only counted the appearances of this character as version compound value. \feedback{why did we look for version placeholders?} \\
%As long as we found only one special character that some software projects use as a version placeholder ('x'), we count only for appearances of this character as version compound value. \\
...
For example a developer might think about two ways of incrementing version 1.0 if he or she needs to come up with a version number for the next release.
 As far as most of the existing conventions describe [LINKS], first way is to assign version 1.1 to new release. Second way is to assign version 2.0. Note that second approach uses increment of first version compound instead of incrementing rightmost version compound, this is megalomaina. 
Megalomania happens because are several ways of incrementing version numbers in cases when version numbers consist of more than one version compound.
